...
columns = [
    'rbt1_scan','rbt2_scan','rbt1_fov','rbt2_fov','rbt1_2_server','rbt2_2_server','server_rcv_1','server_rcv_2','global_map','rrt_rbt1','rrt_rbt2',
    'server_2_rbt1','server_2_rbt2','rbt1_rcv','rbt2_rcv','rbt1_actuation_strt','rbt1_actuation_finish','rbt2_actuation_strt','rbt2_actuation_finish'
]
...
def callback2(message1: Odometry):
   ...
   # print(message1.pose.pose.position)
    ...
    loc = 'robot loc : (' + str(round(message1.pose.pose.position.x)) + ',' + str(round(message1.pose.pose.position.y))   +')'
...
def main():
    ...
    pub3 = rospy.Publisher('/rb1_move_dir', String, queue_size=10)
    pub4 = rospy.Publisher('/robot1_text', String, queue_size=10)
    ...
    while not rospy.is_shutdown():
        ...
        if rbot1_ldr is not None and loc is not None:
            ...
            if(flag_1==True or robot1_status==3 or robot1_status==4):
                # print("sdghis")
                ldata_45 = []
                ldata_all = []
                # print(rbot1_ldr)
                ...
                lidar_data = 'Lidar data : (' + str(int(obstcle[0])) +','+ str(int(obstcle[1]))  +','+ str(int(obstcle[2]))  +','+ str(int(obstcle[3]))  +','+ str(int(obstcle[4]))  +','+ str(int(obstcle[5]))  +','+ str(int(obstcle[6]))  +','+ str(int(obstcle[7]))  +')'
                # print(loc)
                # print(lidar_data)
                ...
                if((cor ==True and (robot1_status==3)) or flag_1):
                    ...
                    # print(lidar_data)
                    # print("robot 1 publish textual description")
                    # user_query = "'''" + in_str + "'''"
                    # user_prompt = in_str
                    ...
                    # result = out['choices'][0]['text'].split("### Response:")[-1]
                    ...
                    if(robot1_status==3 or robot1_status==4):
                        column_name = 'rbt1_actuation_finish'  
